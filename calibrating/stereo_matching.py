...
class SemiGlobalBlockMatching(MetaStereoMatching):
    ...
    def __call__(self, img1, img2):
        ...
        # to gray
        # if simg1.shape != 3:
        #     simg1 = cv2.cvtColor(simg1, cv2.COLOR_GRAY2BGR)
        # elif simg2.shape != 3:
        simg2 = cv2.cvtColor(simg2, cv2.COLOR_GRAY2BGR)
...
if __name__ == "__main__":
    ...
    os.path.join(
        ...
        "C:/Users/scienkong_robot/Desktop/calib",
    ...
    caml = calibrating.Cam(glob(f"{checkboard_img_dir}/new7/*/L.bmp"), board)
    print('caml:', caml)
    camr = calibrating.Cam(glob(f"{checkboard_img_dir}/new7/*/R.bmp"), board)
    print('camr:', camr)
    ...
    feature_matching = calibdiff.LoftrFeatureMatching(dict(shape=(1024, 800)))
    ...
    showb-vis_depth(depth, fix_range=(0.6, 3.5))
